...
public class Robot extends TimedRobot {
  ...
  public class Shooter{
    ...
    public void check(){
      ...
      if(gunner.rhino_speedup()){
        // if(rhino.getVelocity()){} <-- idk if we need it but I think we might
        // go up by a certain % when pressed
        // might need a true/false statement for when button is pressed
      }
      else if(){
        // keep speed where it is
      }
      else{
        // set_motors(0);
        // Do we need to come to a dead stop or should we make it so it just stayes
      }

      if(gunner.rhino_slowdown()){
        // go down by a certain % when pressed
      }
      else if(){
        // keep speed where it is
      }
